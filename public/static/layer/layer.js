  ...
  window.layer = {
    ...
    confirm: function(conMsg, conYes, conTit, conNo){
      return $.layer({
        ...
        title : conTit,
        close : typeof(conNo) == 'function' ? conNo : function(index){ layer.close(index);}
      });
    },
...
//创建骨架
  Class.pt.creat = function(){
    ...
    layerE.css({left: _left + config.border[0], width: config.area[0], height: config.area[1], minWidth:config.minWidth});
  ...
  Class.pt.callback = function(){
    ...
    this.layerS.off('click').on('click', function(e){
      ...
      if (that.config.shadeClose) {
        typeof(that.config.close) == 'function' && that.config.close();
        layer.close(that.index);
      }
    });
