...
class Article_Model extends Base_model
{
   ...
   * @var string
   */
  public $article_sort_table = 'article_sort';
...
EOF;
  ...
  {
    $data = (array)$data;
    $result = $this->db->insert($this->article_table, $data);
    if ($result)
    {
   ...
   * @param null $table
   */
  public function update_article($where, $data, $table = NULL)
  {
    is_null($table) && $table = $this->article_table;
   ...
   * @return mixed
   */
  public function get_article_page_list($extend = '', $limit = 18)
  {
    $this->db->start_cache();
    $this->db->from($this->article_table);
    $this->db->join($this->article_sort_table, $this->article_sort_table.'.id='.$this->article_table.'.sort_id', 'left');
    $extend && $this->db->where($extend, null, false);
    $count = $this->db->count_all_results();

    $this->load->library('HyPage');
    $this->hypage->init($count, $limit);
    $this->db->select($this->article_sort_table.'.sort_name');
    $this->db->select($this->article_table.'.*');
    ...
    $data['list'] = $list;
    $data['page'] = $this->hypage->show($this->config->config['base_url'].'/admin/article');
    $data['count'] = $count;
    return $data;
  }
   ...
   * @return mixed
   */
  public function get_article_list($extend, $limit=18, $order_by='article.id desc')
  {
    $this->db->select($this->article_sort_table.'.sort_name');
    $this->db->select($this->article_table.'.*');
    $this->db->from($this->article_table);
    ...
    if (is_array($extend))
    {
      $this->db->where($extend);
    } elseif(is_string($extend))
    {
      $this->db->where($extend, null, false);
    }
  ...
  {
    $result = $this->db->insert($this->article_sort_table, $data);
    if ($result)
    {
      return $this->db->insert_id();
    }
  ...
  {
    $this->db->from($this->article_sort_table);
    $this->db->where($where);
    return $this->db->delete();
  }
  ...
  {
    $this->table = $this->article_sort_table;
    return $this->get_row_array(array('id'=>$id));
  }
