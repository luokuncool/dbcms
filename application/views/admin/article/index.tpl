  ...
  <div id="all">
    <form id="form" action="" method="get">
      <div class="fr btnBox1 pl10">
        <input type="submit" class="btn_text" value="文章搜索" />
      </div>
        ...
        {if count($list)}
          <table class="w table">
            ...
            {foreach $list as $article}
              <tr>
                <th>
                  <input id="cb-select-5" type="checkbox" name="id[]" value="{$article.id}" />
                  <div class="locked-indicator"></div>
                </th>
                <td>
                  <a class="a-black-n" href="{$base_url}/admin/article/edit_article/{$article.id}?redirect={$active_url}">{$article.article_name}</a>
                </td>
                <td>admin</td>
                <td>{$article.sort_name}</td>
                <td>{if $article['status']=='1'}发布{else}草稿{/if}</td>
                <td>{$article.add_time|date_format:'Y-m-d H:i:s'}</td>
                <td>
                  <a class="a-black-n" href="{$base_url}/admin/article/edit_article/{$article.id}?redirect={$active_url}">编辑</a>
                </td>
              </tr>
            {/foreach}
            </tbody>
  ...
  $(function(){
    ...
    form.bind('submit', function(){
      ...
      if (action != -1) {
        switch (action) {
          case 'delete' :
            url = '{$base_url}/admin/article/delete_article';
            break;
        }
        layer.load('请稍等...');
