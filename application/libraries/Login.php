...
class Login
{
    ...
    public function init($keyName = '', $tokenTimeout = '', $sidTimeOut = '', $isThisIp = '', $isThisBrowser = '')
    {
        ...
        if ($isThisIp != '') {
            $this->isThisIp = (bool)$isThisIp;
        }
        if ($isThisBrowser != '') {
            $this->isThisBrowser = (bool)$isThisBrowser;
        }
        //cookie、session超时时间初始化
        if (intval($tokenTimeout) > 0) {
            $this->tokenTimeout = intval($tokenTimeout);
        }
        if (intval($sidTimeOut) > 0) {
            $this->sidTimeOut = intval($sidTimeOut);
        }
    }
